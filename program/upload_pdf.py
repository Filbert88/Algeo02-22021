...
if result_data:
    pdf_filename = "similarity_report.pdf"
    doc = SimpleDocTemplate(os.path.join('img',pdf_filename), pagesize=letter,author ="PixPlore.")

    pdf_content = []

    font_name = "SpaceMono-Regular"  
    font_path = "fonts/SpaceMono-Regular.ttf" 
    pdfmetrics.registerFont(TTFont(font_name, font_path))

    styles = getSampleStyleSheet()
    title_style = styles["Title"]
    title_style.fontName = "SpaceMono-Regular"
    ...
    title = Paragraph("SIMILARITY REPORT", title_style)
    pdf_content.append(title)
    pdf_content.append(Spacer(1, 20))
    pdf_content.append(HorizontalLine(doc.width))
    pdf_content.append(Spacer(1, 20))

    input_image_url = result_data[0].get("image_url")
    input_img = PILImage.open(input_image_url)
    input_img = input_img.resize((150, 150))
    input_suffix = os.path.splitext(input_image_url)[1].lower()
